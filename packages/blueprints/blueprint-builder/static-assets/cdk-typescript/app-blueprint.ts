...
import { SourceRepository, SourceFile, StaticAsset, SubstitionAsset } from '@amazon-codecatalyst/blueprint-component.source-repositories';
...
export interface Options extends ParentOptions {
  ...
  /**
   * @collapsed false
   * @displayName CDK workflow settings
   */
  cdkSettings: {
    /**
    ...
    region: string;
    /**
     * What do you want to name the AWS CloudFormation Stack?
...
export class Blueprint extends ParentBlueprint {
  constructor(options_: Options) {
    ...
    const app = new SubstitionAsset('bin/app.ts');
    new SourceFile(repository, app.path(), app.substitute({
    ...
    workflowBuilder.setName('cdk-app-workflow');
    ...
    workflowBuilder.addCdkBootstrapAction({
      ...
      configuration: {
        Region: options.cdkSettings.region,
      }
    });
