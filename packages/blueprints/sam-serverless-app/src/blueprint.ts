  ...
  override synth(): void {
    ...
    addGenericBuildAction(
      ...
      [
        ...
        {Run: 'sam build'},
        {
          Run: `sam package --template-file ./.aws-sam/build/template.yaml --resolve-s3 --output-template-file output.yaml --region ${region}`,
        },
      ],
      artifactName,
    );
    ...
    const readmeContents = generateReadmeContents(
      ...
      this.options.workflow.cloudFormationStackName,
      workflowName,
    );
