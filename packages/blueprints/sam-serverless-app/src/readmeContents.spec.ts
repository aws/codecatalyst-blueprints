...
describe('readme contents', () => {
  describe('happy case', () => {
    ...
    it('incorporates runtime param', () => {
      expect(readmeContents).toContain(
        '$ gradle test');
    });

    it('incorporates runtimeMapping param', () => {
      expect(readmeContents).toContain(
        `${runtimeMapping.srcCodePath} - Code for the Lambda function`);
    });

    it('incorporates defaultReleaseBranch param', () => {
      expect(readmeContents).toContain(
        'The workflow is triggered by pushes to the `test-release-branch` of the source repository');
    });

    it('incorporates lambdas param', () => {
      expect(readmeContents).toContain(
        'lambda1, lambda2 - Source code');
    });

    it('incorporates environment param', () => {
      expect(readmeContents).toMatch(
        /`test-env-name` using the cloudformation stack `\S+-test-env-name`/);
    });

    it('incorporates cloudFormationStackName param', () => {
      expect(readmeContents).toContain(
        'using the cloudformation stack `test-cfn-stack-name-');
    });

    it('incorporates workflowName param', () => {
      expect(readmeContents).toContain(
        '.codecatalyst/workflows/test-workflow-name.yaml - The template');
      expect(readmeContents).toContain(
        'using the workflow defined in `.codecatalyst/workflows/test-workflow-name.yaml`.');
    });
  });
